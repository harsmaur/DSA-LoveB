...
void InsertionSort(int arr[], int n){
    for(int i =0; i<n; i++){
        ...
        int j = i-1; //j is running reverse from i-1 to 0 
         ...
         for(; j>=0; j--){  // Inner loop to find the correct position for the current element by comparing it with the previous elements.
            if(arr[j] > temp){ // If the previous element is greater than the current element, shift the previous element to the right.
